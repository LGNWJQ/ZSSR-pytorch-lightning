...
if __name__ == "__main__":
    ...
    for i in range(10):
        for batch in dataloader2:
            hr, lr = batch
            hr, lr = hr[0], lr[0]
            save_image(hr, f'./loader/hr{i}.png')
            save_image(lr, f'./loader/lr{i}.png')

    # image = imageio.v2.imread(image_path)
    # h, w, _ = image.shape
    # short_side = min(h, w)
    # max_scale = min(5, short_side / patch_size)
    # scale_list = np.linspace(1/max_scale, 1, num_scale+1)[1:]
    # pdf = np.power(scale_list, 2)
    # pdf = pdf / np.sum(pdf)
    #
    # h_list = (h * scale_list).astype(np.uint16)
    ...
    #     image_list.append(A.resize(image, height=h_list[i], width=w_list[i]))
    ...
    #
    # for i in range(10):
    #     sample_index = np.random.choice(range(num_scale), batch_size, p=pdf)
    #     image_sample = [image_list[i] for i in sample_index]
    #
    #     hr_sample = [transform(image=img)['image'] for img in image_sample]
    #     lr_sample = [A.resize(img, patch_size//sr_factor, patch_size//sr_factor) for img in hr_sample]
    #     lrup_sample = [A.resize(img, patch_size, patch_size, interpolation=cv2.INTER_CUBIC) for img in lr_sample]
    #
    #     hr_tensor = [to_tensor(image=img)['image'].unsqueeze(0) for img in hr_sample]
    #     lrup_tensor = [to_tensor(image=img)['image'].unsqueeze(0) for img in lrup_sample]
    #     hr_batch = torch.cat(hr_tensor, dim=0)
    #     lrup_batch = torch.cat(lrup_tensor, dim=0)
    #
    #     print(hr_batch.shape, lrup_batch.shape)
    #     print(hr_batch.min(), lrup_batch.max())
    #     print(hr_batch.dtype, lrup_batch.dtype)
    #     save_image(hr_batch, f'./sample/hr{i}.png')
    #     save_image(lrup_batch, f'./sample/lr{i}.png')
    #
    # print(h, w)
    # print(max_scale)
    # print(scale_list)
    ...
    DATA1
    '''
    # dataset1 = Single_Image(image_path, sr_factor)
    ...
    # )
    # for i in range(10):
    #     for idx, batch in enumerate(dataloader1):
    #         lr_upscale, lr = batch
    #         print(lr_upscale.shape, lr.shape)
    #         print(lr.min(), lr.max())
    #         print(lr.dtype)
    #         save_image(lr_upscale, f'./{i}.png')
